...
Pick pickUnder(const Staff[] staff, const size_t[] order, const(string)[] pool, double threshold) @safe {
  Pick p;
  foreach (k; order) {
    if (staff[k].total >= threshold) { continue; }
    if (p.any == size_t.max || staff[k].total < staff[p.any].total) { p.any = k; }
    if ((pool.length == 0 || pool.canFind(staff[k].discipline)) && (p.inPool == size_t.max || staff[k].total < staff[p.inPool].total)) {
      p.inPool = k;
    }
  }
  return p;
...
size_t total(const Staff s) pure nothrow @safe @nogc { return s.existing + s.added; }

/** Split one TSV line into fields, preserving empty trailing cells. */
string[] fields(string line) pure @safe { return line.split("\t"); }
...
const(string)[] poolFor(string programme, const Rule[] rules) @safe {
  ...
  foreach (ref r; rules) { if (p.indexOf(r.keyword) >= 0) { return r.pool; } }
  return null;
}

/** True for programmes that do not receive a personal tutor (e.g. NEBOSH). */
bool isExcluded(string programme) @safe { return programme.toLower.indexOf("nebosh") >= 0; }
...
int main(string[] args) {
  ...
  // existing load = non-NEBOSH students whose PT is a real staff ID (kept as-is)
  size_t tutoredCount;
  foreach (ref row; rows) {
    if (isExcluded(row[progCol])) { continue; } // NEBOSH students do not receive a personal tutor
    tutoredCount++;
  ...
  auto geo = ["Physical Geography and Environmental Science", "Cold and Palaeo Environments"];
  const Rule[] rules = [
  ...
  // keep every tutor within mean +/- 5 of the team mean (NEBOSH excluded from the load)
  const meanLoad = cast(double) tutoredCount / staff.length;
  ...
  foreach (i, ref row; rows) {
    if (isExcluded(row[progCol])) { continue; } // NEBOSH: no personal tutor, leave New tutor ID blank
    const pt = row[ptCol].strip;
    if (pt in validId) { continue; }
    needing++;
    ...
    if (below.any != size_t.max) {
      ...
      if (under.any != size_t.max) {
        ...
        foreach (k; staffOrder) { if (staff[k].total < staff[best].total) { best = k; } }
      }
    }
    if (pool.length && !pool.canFind(staff[best].discipline)) { spilled++; } // team was full within the band, assigned outside to keep balance
    staff[best].added++;
  ...
  foreach (i, ref row; rows) { writeln((row ~ newCol[i]).join("\t")); }

  // --- diagnostics to stderr ---
  stderr.writefln("Students: %d", rows.length);
  stderr.writefln("NEBOSH excluded (no personal tutor): %d", rows.length - tutoredCount);
  stderr.writefln("Tutored students: %d", tutoredCount);
  stderr.writefln("Kept (valid PT): %d", tutoredCount - needing);
  stderr.writefln("Reassigned: %d (of which spilled outside their team to keep balance: %d)", needing, spilled);
  ...
  staffOrder.sort!((a, b) => staff[a].existing + staff[a].added > staff[b].existing + staff[b].added);
  stderr.writeln("discipline\tid\texisting\tnew\ttotal");
  foreach (k; staffOrder) {
    stderr.writefln("%s\t%s\t%d\t%d\t%d", staff[k].discipline, staff[k].id, staff[k].existing, staff[k].added, staff[k].existing + staff[k].added);
  }
...
ptrdiff_t countUntilName(const string[] header, string name) pure @safe nothrow {
  foreach (i, h; header) { if (h.strip == name) { return i; } }
  return -1;
}
